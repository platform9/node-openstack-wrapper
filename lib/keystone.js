...
Keystone.prototype.listEndpoints = function(project_token, service_id, cb)
{
  var self = this;
  var request_options = this.getRequestOptions(project_token, '/endpoints?service_id=' + service_id, true);
  request_options.metricPath = 'remote-calls.keystone.endpoints.list';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'endpoints';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    for(var n = 0; n < body.endpoints.length; n++)
    {
      endpoints_array[n] = self.mangleObject('Endpoint', body.endpoints[n]);
    }

    //tack these on for easy consupmtion and in case we ever need pagination
    endpoints_array.self = body.links.self;
    endpoints_array.previous = body.links.previous;
    endpoints_array.next = body.links.next;

    cb(null, endpoints_array);
  });
};


Keystone.prototype.listServices = function(project_token, type, cb)
{
  var self = this;
  var request_options = this.getRequestOptions(project_token, '/services?type=' + type, true);
  request_options.metricPath = 'remote-calls.keystone.services.list';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'services';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    for(var n = 0; n < body.services.length; n++)
    {
      services_array[n] = self.mangleObject('Service', body.services[n]);
    }

    //tack these on for easy consupmtion and in case we ever need pagination
    services_array.self = body.links.self;
    services_array.previous = body.links.previous;
    services_array.next = body.links.next;

    cb(null, services_array);
  });
};
